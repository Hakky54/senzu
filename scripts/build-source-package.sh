...
cp binary/amd64/senzu "${SOURCE_DIR}/binaries/senzu-amd64"
cp binary/aarch64/senzu "${SOURCE_DIR}/binaries/senzu-arm64"
...
Description: A CLI tool to get the battery percentage
EOF
...
override_dh_auto_install:
	install -D -m 755 binaries/senzu-$(DEB_HOST_ARCH) debian/senzu/usr/bin/senzu
...
senzu (${VERSION}-1~${DISTRO}) ${DISTRO}; urgency=medium
...
Source: https://github.com/Hakky54/senzu
...
senzu source: source-is-missing [binaries/senzu-amd64]
senzu source: source-is-missing [binaries/senzu-arm64]
EOF
